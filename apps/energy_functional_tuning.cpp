/**
 * Small experiments for tuning the energy functional baseline.
 ...
 * @param wallPointsTerm Is the wall points term being included?
 *
 * @return outFile The results output file
 */
std::filesystem::path getOutputFile(const std::filesystem::path &outDir,
                                    const bool &wallPointsTerm) {

  if (wallPointsTerm) {
    return outDir / ("energy_functional_with_wall_points_term_results.csv");
  } else {
    return outDir / ("energy_functional_no_wall_points_term_results.csv");
  }
}
...
std::shared_ptr<CoverageRobot> getRobot(
    const bool &wallPointsTerm, const int &timeBound,
 ...
 *
 * @param wallPointsVals The wall points term values being tested
 ...
 */
void runExperiments(const std::vector<bool> &wallPointsVals,
                    const std::vector<std::string> &envs,
  ...
  // Iterate through wallPointsVals candidates
  for (const int &wallPointsTerm : wallPointsVals) {
    std::cout << "WALL POINTS TERM BEING USED: " << wallPointsTerm << "\n";

    // Prepare output
    std::vector<std::vector<CoverageResult>> resultsForMethod{};
    std::filesystem::path outFile{getOutputFile(outDir, wallPointsTerm)};
    ...
    for (int envNum{0}; envNum < envs.size(); ++envNum) {
      ...
      std::shared_ptr<CoverageRobot> robot{
          getRobot(wallPointsTerm, timeBounds.at(envNum), fov, exec, dim,
                   groundTruthIMac)};
...
int main() {
  ...
  std::vector<bool> wallPointsVals{false, true};
  ...
  // Output directory
  std::filesystem::path outDir{"../../data/results/energy_functional_tuning"};

  // Number of runs
  int numRuns{10};

  // RUn the experiments
  runExperiments(wallPointsVals, envs, timeBounds, fov, inDir, outDir, numRuns);
}